...
function render_related_content(string $type, string $slug): void {
  ...
  ?>
<style>
.rc-section{padding:72px 48px;border-top:1px solid var(--border,rgba(0,0,0,.07));background:var(--bg,#f5f5f3)}
...
.rc-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(220px,1fr));gap:1px;background:var(--border,rgba(0,0,0,.07));border:1px solid var(--border,rgba(0,0,0,.07));border-radius:16px;overflow:hidden}
.rc-col{background:#fff;padding:24px;display:flex;flex-direction:column;gap:14px}
...
.rc-item{display:flex;align-items:center;gap:12px;padding:10px;border-radius:10px;text-decoration:none;border:1px solid transparent;transition:background .18s,border-color .18s;cursor:pointer}
...
.rc-item__thumb{width:56px;height:36px;object-fit:cover;border-radius:6px;flex-shrink:0}
